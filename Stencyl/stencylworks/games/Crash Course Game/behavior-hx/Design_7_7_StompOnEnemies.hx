...
class Design_7_7_StompOnEnemies extends ActorScript
{
					...
					if(event.thisFromBottom)
					{
						if((internalGetGroup(event.otherActor, event.otherShape, event) == _StompableGroup))
						{
							if(!(asBoolean(actor.getLastCollidedActor().getActorValue("_BeingStomped"))))
							{
								...
								{
									actor.applyImpulse(0, -1, actor.getLastCollidedActor().getValue("Stompable", "_PushPlayerJumpForce"));
								}
								else
								{
									actor.applyImpulse(0, -1, actor.getLastCollidedActor().getValue("Stompable", "_PushPlayerForce"));
								}
							}
						}
					}
