...
class HyperTouch extends EventDispatcher{
	...
	private static var hypTouch_init = Lib.load( "HyperTouch" , "hyp_touch_init" , 0 );
	...
	// -------o protected
		...
		*/
		...
		private function _onDispatchPan( iPhase : Int , tx : Float , ty : Float , vx : Float , vy : Float , fCenterX : Float , fCenterY : Float ) : Void{
			...
			#if android
			var ev = new GesturePanEvent( -tx , -ty , vx / Lib.current.stage.stageWidth , vy / Lib.current.stage.stageHeight , fCenterX , fCenterY);
